...
type Scritto = (String,[Double],[Double],[Double])
...
s_alloc :: Transport t => t -> (Scritto,Int) -> IO ()
s_alloc fd (s,b) = do
  let s_msg n (_,fr,am,bw) = b_setn1 n 0 (fr ++ am ++ bw)
  _ <- async fd (b_alloc b 15 1)
  send fd (s_msg b s)

s_init :: Transport t => t -> IO ()
s_init fd = mapM_ (s_alloc fd) (zip scritto [0..])
