...
>                       , 24.93169593811035 ] )
>         ; (cf, ca, _) = crotale
>         ; fn m f a = let { r0 = rand (f+0) 0 (2 * pi)
>                          ; r1 = rand (f+1) 0.1 0.3
>                          ; r2 = rand (f+2) 0.025 0.1
>                          ; r3 = rand (f+3) 0 (2 * pi)
>                          ; o = sinOsc AR (f * midiRatio m) r0
>                          ; a' = a * sinOsc KR r1 0 }
>                      in pan2 o (sinOsc KR r2 r3) a'
>         ; oo = let { x = control KR "m" 1 }
>                in out 0 (sum (zipWith (fn x) cf ca) * 0.1)
>         }
>   in withSC3 (\fd -> do { _ <- async fd (d_recv (synthdef "c" oo))
>                         ; send fd (s_new "c" (-1) AddToTail 1 [("m",-12)])
>                         ; send fd (s_new "c" (-1) AddToTail 1 [("m",-9)])
>                         })
