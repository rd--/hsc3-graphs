...
diffraction_q :: UId m => m UGen
diffraction_q = do
  n <- lfNoise0M KR 128
  s <- diffraction_p
  return (combN s 0.2 (n * 0.1 + 0.1) 3)

diffraction_r :: UId m => m UGen
diffraction_r =
...
diffraction_m = fmap sum (sequence [diffraction_p,diffraction_q,diffraction_r])
