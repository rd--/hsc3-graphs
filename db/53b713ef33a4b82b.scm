; DegreeToKey ; note that Add3 is allowed where Sum3 is not (Sum3 requires all rates to be equal else it crashes the server)
(let* ((b (asLocalBuf (list 0 2 3.2 5 7 9 10)))
       (x (MouseX 0 15 0 0.1))
       (k (DegreeToKey b x 12))
       (c (lambda (n r)
            (let* ((o (Mul (SinOsc (MidiCps (Add3 r k n)) 0) 0.1))
                   (t (LfPulse (MidiCps (List 48 55)) 0.15 0.5))
                   (f (MidiCps (MulAdd (SinOsc 0.1 0) 10 r)))
                   (d (Mul (Rlpf t f 0.1) 0.1))
                   (m (Add o d)))
              (Add (CombN m 0.31 0.31 2) m))))
       (n (Mul (LfNoise1 (List 3 3.05)) 0.04)))
