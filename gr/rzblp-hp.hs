...
hpb q =
    let g _ = do
          return (blip AR #(lfn q 1330 1395) 24 * #(lfn q 0.001 0.007))
    in mixFillM 2 g
...
mk_f d = do
  ...
  s2 <- resonz (impulse AR q (mce2 0 0)) #(lfn 5 30 640) #(lfn q 0.1 0.5) *. lfn q 0.01 1.8
  s3 <- hpb q .*. lfn q 1.2 1.8
  s4 <- return (blip AR #(lfn q 16 36) 3 * mce2 0.03 0.09)
