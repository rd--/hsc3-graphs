...
--
-- dust triggers random re-indexing of buffers
tr_nd :: ID z => z -> UGen -> (UGen, UGen)
tr_nd z n =
  let t = dust (z,'α') KR (control KR "tr_rate" 1.6)
      r1 = tRand (z,'β') 0 6 t
      r2 = tRand (z,'γ') 0 6 t
      r3 = tRand (z,'δ') 0 6 t
      r4 = tRand (z,'ε') 0 6 t
      mnn = bufRd 1 KR 0 r1 NoLoop NoInterpolation
      loc = bufRd 1 KR 1 r2 NoLoop NoInterpolation
      amp = bufRd 1 KR 2 r3 NoLoop NoInterpolation
      stp = bufRd 1 KR 3 r4 NoLoop NoInterpolation
  in (pan2 (sig_1 stp mnn) loc amp
     ,tr_1 t n (mnn,loc,amp,stp))

tr_out_degree :: Num n => n
tr_out_degree = 6

-- > putStrLn $ synthstat tr_out
tr_out :: UGen
tr_out =
  let ns = zipWith tr_nd ['α' .. ]  [1 .. tr_out_degree]
      o = sum (map fst ns)
  in (mrg (o : map snd ns))
